...
const RoomIndex = () => {
  ...
  const getAllRooms = () => {
    client.get({ endpoint: 'shuffle_rooms' })
    .then((res) => setRooms(res.contents))
  }
  useEffect(() => {
    getAllRooms()
  }, [])
              ...
              <Card>
                <Card.Body>
                  ...
                  <div className="d-flex justify-content-between">
                    <Button variant="info" href={`/rooms/${room.id}`}>シャッフル開始</Button>
                    <Button variant="outline-danger" size="sm" onClick={(e) => handleDelete(room.id, e)}>削除</Button>
                  </div>
                </Card.Body>
              </Card>
            </Col>
